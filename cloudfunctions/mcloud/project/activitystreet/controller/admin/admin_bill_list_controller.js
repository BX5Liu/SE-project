...
const BillService = require('../../service/admin/admin_bill_service.js');
const ActivityService = require('../../service/pay_service.js');
...
class AdminActivityController extends BaseProjectAdminController {
	...
	async getAdminActivityList() {
		...
		let adminService = new BillService();
    ...
    console.log('Bill inserted successfully:');
		...
		// 	stop: 'must|string|name=截止时间',
		// 	bills: 'must|int|name=费用',
		// };
		...
		let service = new BillService();
		let result = await service.GoinsertBills(input);

		this.logOther('添加了账单《' + input.title + '》');
